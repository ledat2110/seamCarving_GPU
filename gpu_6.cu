...
__global__ void getMinColSeam (int *in, int width, int *out){
    ...
    if (i + 1 < width){
        s_mem[threadIdx.x + 1 + blockDim.x * 2] = threadIdx.x + 1;
    ...
    for (int stride = 1; stride < 2 * blockDim.x; stride *= 2){
      if (threadIdx.x % stride == 0){
        if (i + stride < width){
            int idx = threadIdx.x + blockDim.x * 2;
            if (s_mem[s_mem[idx]] > s_mem[s_mem[idx + stride]]){
    ...
    if (threadIdx.x == 0){
      out[blockIdx.x] = s_mem[blockDim.x * 2] + blockDim.x * 2 * blockIdx.x;
    }
  }
